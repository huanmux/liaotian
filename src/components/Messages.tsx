...
import { supabase, type Message, type Profile, uploadMedia, type MessageReaction } from "../lib/supabase"
...
type AppMessage = Message & {
  ...
  reactions?: any[] // Add reactions to the message type
}
...
const AudioPlayer: React.FC<AudioPlayerProps> = ({ src, isOutgoing }) => {
  ...
  return (
    <motion.div
      ...
      className="flex items-center space-x-2 w-full max-w-full mb-1"
    >
      ...
      <button
        ...
        style={{
          backgroundColor: isOutgoing ? "rgba(var(--color-text-on-primary), 0.15)" : "rgb(var(--color-surface-hover))",
          color: primaryColor,
        }}
      >
...
export const Messages = ({
    ...
    async (messageId: string, emoji: string) => {
      ...
      if (existingReaction) {
        if (existingReaction.emoji === emoji) {
          ...
          await supabase.from("message_reactions").update({ emoji: emoji }).eq("id", existingReaction.id)
        }
      } else {
  ...
  useEffect(() => {
    ...
    outgoingTypingChannelRef.current.subscribe(async (status) => {
  ...
  const loadMessages = async (recipientId: string) => {
    ...
    const { data: messagesData, count } = await supabase
      ...
      .select(
        `
        id, sender_id, recipient_id, content, created_at, media_url, media_type, read, reply_to_id, is_deleted, is_edited,
    ...
    const repliesMap = new Map<string, AppMessage["reply_to"]>()
  ...
  const loadMoreMessages = async () => {
    ...
    if (
      !messagesData ||
      messagesData.length < MESSAGE_PAGE_SIZE ||
      (count !== null && messages.length + (messagesData?.length || 0) >= count)
    ) {
      setHasMoreMessages(false)
    }
  ...
  const getPreview = () => {
    if (file) {
      ...
      if (file.type.startsWith("image/")) {
        return <img src={url || "/placeholder.svg"} className="max-h-24 rounded-lg shadow-sm" alt="Preview" />
      }
    ...
    if (remoteUrl) {
      if (remoteUrl.match(/\.(jpeg|jpg|gif|png|webp)$/i)) {
        return (
          <img src={remoteUrl || "/placeholder.svg"} className="max-h-24 rounded-lg shadow-sm" alt="Remote preview" />
        )
      }
    ...
    return (
      <div className="flex h-full bg-[rgb(var(--color-background))] flex-col md:flex-row text-[rgb(var(--color-text))]">
        ...
        <div className="md:hidden flex-shrink-0 p-2 bg-[rgb(var(--color-surface))] border-b border-[rgb(var(--color-border))] flex gap-2 z-50">
          ...
          <button className="flex-1 p-2 rounded font-bold text-sm bg-[rgb(var(--color-primary))] text-[rgb(var(--color-text-on-primary))]">
            Gazebos
          </button>
        </div>
  ...
  return (
    <div className={`flex h-full bg-[rgb(var(--color-background))] overflow-hidden ${activeTab === 'gazebos' ? 'flex-col' : ''}`}>
      {/* Sidebar */}
        ...
        <AnimatePresence mode="wait">
          {(showSidebar || window.innerWidth > 768) && (
            <motion.div 
              ...
              className="w-64 md:w-72 bg-[rgb(var(--color-surface))] border-r border-[rgb(var(--color-border))] flex flex-col overflow-hidden"
            >
      ...
      {/* Main Chat Area */}
      <div className="flex-1 flex flex-col overflow-hidden bg-[rgb(var(--color-background))]">
        {selectedUser ? (
          <>
            {/* Header */}
            <motion.div 
              ...
              className="flex items-center justify-between p-3 sm:p-4 bg-[rgb(var(--color-surface))] border-b border-[rgb(var(--color-border))] flex-shrink-0"
            >
                ...
                <button onClick={() => goToProfile(selectedUser.id)} className="flex items-center gap-3 flex-1 min-w-0 group">
                  <div className="relative">
                    <img
                      ...
                      className="w-10 h-10 rounded-full object-cover bg-[rgb(var(--color-background))] ring-2 ring-transparent group-hover:ring-[rgb(var(--color-accent))] transition"
                      alt=""
                    />
            ...
            {replyingTo && (
              <motion.div 
                ...
                transition={{ duration: 0.3 }}
                className="px-3 sm:px-4 py-2 bg-[rgb(var(--color-surface-hover))] border-b border-[rgb(var(--color-border))] flex items-center justify-between flex-shrink-0"
              >
                <div className="flex items-center gap-2 px-3 py-2 bg-[rgb(var(--color-surface))] rounded-xl w-full border-l-4 border-[rgb(var(--color-accent))]">
                    <div className="flex-1 min-w-0 overflow-hidden">
                        <div className="text-xs font-bold text-[rgb(var(--color-accent))] mb-0.5 truncate">
                  ...
                  >
                      {isMe && (
                          <motion.div 
                            ...
                            transition={{ duration: 0.2 }}
                            className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition duration-200 mr-2 self-center"
                          >
                              <motion.button 
                                whileHover={{ scale: 1.15 }}
                                whileTap={{ scale: 0.95 }}
                                onClick={(e) => { const rect = e.currentTarget.getBoundingClientRect(); setReactionMenu({ messageId: msg.id, x: rect.left, y: rect.top, isOutgoing: true }); }} 
                                className="p-1.5 rounded-full text-[rgb(var(--color-text-secondary))] hover:bg-[rgb(var(--color-surface-hover))]"
                              >
                                  <Smile size={14} />
                              </motion.button>
                              <motion.button 
                      ...
                      {msg.media_url && (
                        <div className={`mb-1 ${msg.content ? "pb-1" : ""}`}>
                          {msg.media_type === 'image' && (
                            <img src={msg.media_url || "/placeholder.svg"} className="rounded-lg w-full h-auto max-h-80 object-cover" alt="Image" loading="lazy" />
                          )}
        ...
        <motion.div 
          ...
          animate={{ y: 0, opacity: 1 }}
          transition={{ duration: 0.3 }}
          className="p-3 sm:p-4 border-t border-[rgb(var(--color-border))] bg-[rgb(var(--color-surface))] flex-shrink-0"
        >
          ...
          <motion.div 
            ...
            className="flex-1 flex items-center justify-center bg-[rgb(var(--color-background))] flex-col gap-6 p-8"
          >
             <motion.div 
               animate={{ y: [0, -8, 0] }}
               transition={{ duration: 3, repeat: Number.POSITIVE_INFINITY }}
               className="w-24 h-24 rounded-full bg-[rgb(var(--color-surface))] flex items-center justify-center shadow-inner"
             >
             ...
             <motion.button 
               ...
               className="md:hidden mt-4 bg-[rgb(var(--color-primary))] text-[rgb(var(--color-text-on-primary))] px-6 py-3 rounded-full font-semibold shadow-lg hover:shadow-xl transition flex items-center gap-2"
             >
      ...
      <AnimatePresence>
        {reactionMenu && (
          ...
          >
            <motion.div 
              className="absolute bg-[rgb(var(--color-surface))] rounded-2xl shadow-xl flex flex-col z-50 pointer-events-auto border border-[rgb(var(--color-border))] min-w-[180px]"
              style={{ 
        ...
        {viewingReactionsFor && (
          ...
          >
              <motion.div 
                ...
                transition={{ type: 'spring', stiffness: 300, damping: 30 }}
                className="bg-[rgb(var(--color-surface))] w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden border border-[rgb(var(--color-border))]"
                onClick={e => e.stopPropagation()}
              >
        ...
        {showMediaGallery && (
          ...
          >
              <motion.div 
                ...
                exit={{ scale: 0.9, rotateX: -10 }}
                transition={{ type: 'spring', stiffness: 300, damping: 30 }}
                className="bg-[rgb(var(--color-surface))] w-full max-w-4xl h-[80vh] rounded-3xl shadow-2xl border border-[rgb(var(--color-border))] overflow-hidden flex flex-col"
                onClick={e => e.stopPropagation()}
              >
                                                  ...
                                                  <div key={msg.id} className="aspect-square relative group bg-[rgb(var(--color-surface))] overflow-hidden rounded-md border border-[rgb(var(--color-border))]">
                                                      {galleryTab === 'image' ? (
                                                          <a href={msg.media_url!} target="_blank" rel="noopener noreferrer" className="block w-full h-full">
                                                              <img src={msg.media_url! || "/placeholder.svg"} className="w-full h-full object-cover hover:scale-105 transition duration-300" alt="Shared" />
                                                          </a>
                                                      ) : (
